...
retry:
	if (error_out)
		strcpy_s(*error_out, 100, "device not found");

	adb_mutex_lock(&transport_lock);
	for (t = transport_list.next; t != &transport_list; t = t->next) {
		if (t->connection_state == CS_NOPERM) {
			if (error_out)
				strcpy_s(*error_out, 100, "insufficient permissions for device");
			continue;
		}
		...
		if (serial) {
			if ((t->serial && !strcmp(serial, t->serial)) ||
				...
				if (result) {
					if (error_out)
						strcpy_s(*error_out, 100, "more than one device");
					ambiguous = 1;
					result = NULL;
					break;
				}
			...
			if (ttype == kTransportUsb && t->type == kTransportUsb) {
				...
				if (result) {
					if (error_out)
						strcpy_s(*error_out, 100, "more than one emulator");
					ambiguous = 1;
					result = NULL;
					break;
				}
				...
				if (result) {
					if (error_out)
						strcpy_s(*error_out, 100, "more than one device and emulator");
					ambiguous = 1;
					result = NULL;
					break;
				}
	...
	if (result) {
		if (result->connection_state == CS_UNAUTHORIZED) {
			if (error_out)
				strcpy_s(*error_out, 100, "device unauthorized. Please check the confirmation dialog on your device.");
			result = NULL;
		}
		...
		if (result && result->connection_state == CS_OFFLINE) {
			if (error_out)
				strcpy_s(*error_out, 100, "device offline");
			result = NULL;
		}
		/* check for required connection state */
		if (result && state != CS_ANY && result->connection_state != state) {
			if (error_out)
				strcpy_s(*error_out, 100, "invalid device state");
	...
	const char *prefix, const char *qual, int sanitize_qual)
{
	...
	len = _snprintf(*buf, *buf_size, "%s%n%s", prefix, &prefix_len, qual);
	...
	int long_listing)
{
	...
	if (!long_listing) {
		return _snprintf(buf, bufsize, "%s\t%s\n", serial, statename(t));
	}
	else {
		size_t len, remaining = bufsize;

		len = _snprintf(buf, remaining, "%-22s %s", serial, statename(t));
		remaining -= len;
		buf += len;
