...
static int adb_qemu_trace_init(void)
{
	...
	_snprintf(con_name, sizeof(con_name), "qemud:adb-debug");
...
void adb_qemu_trace(const char* fmt, ...)
{
	...
	if (adb_debug_qemu >= 0) {
		v_snprintf(msg, sizeof(msg), fmt, args);
...
{
#if ADB_HOST
	return _snprintf(buf, bufsize, "host::") + 1;
#else
	...
	len = _snprintf(buf, remaining, "%s::", adb_device_banner);
	remaining -= len;
	buf += len;
	for (i = 0; i < num_cnxn_props; i++) {
		char value[PROPERTY_VALUE_MAX];
		property_get(cnxn_props[i], value, "");
		len = _snprintf(buf, remaining, "%s=%s;", cnxn_props[i], value);
		remaining -= len;
		buf += len;
	}
...
static void send_msg_with_header(int fd, const char* msg, size_t msglen) {
	...
	if (msglen > 0xffff)
		msglen = 0xffff;
	_snprintf(header, sizeof(header), "%04x", (unsigned)msglen);
	writex(fd, header, 4);
...
static void send_msg_with_okay(int fd, const char* msg, size_t msglen) {
	...
	if (msglen > 0xffff)
		msglen = 0xffff;
	_snprintf(header, sizeof(header), "OKAY%04x", (unsigned)msglen);
	writex(fd, header, 8);
	writex(fd, msg, msglen);
}
...
static int format_listener(alistener* l, char* buffer, size_t buffer_len) {
	...
	if (buffer != NULL) {
		_snprintf(buffer, buffer_len, "%s %s %s\n",
			l->transport->serial, l->local_name, l->connect_to);
	}
	// NOTE: _snprintf() on Windows returns -1 in case of truncation, so
...
int launch_server(int server_port)
{
	...
	if (pid == 0) {
		...
		char str_port[30];
		_snprintf(str_port, sizeof(str_port), "%d", server_port);
		// child process
...
int handle_forward_request(const char* service, transport_type ttype, char* serial, int reply_fd)
{
	...
	if (!strncmp(service, "forward:", 8) ||
		...
		transport = acquire_one_transport(CS_ANY, ttype, serial, (char**)&err);
		if (!transport) {
...
int handle_host_request(char *service, transport_type ttype, char* serial, int reply_fd, asocket *s)
{
	...
	if (!strncmp(service, "transport", strlen("transport"))) {
		char error_string[100] = "unknown failure";
		...
		transport = acquire_one_transport(CS_ANY, type, serial, (char**)&error_string);
	...
	if (!strncmp(service, "get-serialno", strlen("get-serialno"))) {
		char out[100] = "unknown";
		transport = acquire_one_transport(CS_ANY, ttype, serial, NULL);
		if (transport && transport->serial) {
			strcpy_s(out , transport->serial);
		}
	...
	if (!strncmp(service, "get-devpath", strlen("get-devpath"))) {
		...
		if (transport && transport->devpath) {
			strcpy_s(out, transport->devpath);
		}
		send_msg_with_okay(reply_fd, out, strlen(out));
		return 0;
	}
