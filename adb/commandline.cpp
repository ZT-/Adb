...
static int send_shellcommand(transport_type transport, char* serial, char* buf)
{
	...
	for (;;) {
		...
		adb_sleep_ms(1000);
		do_cmd(transport, serial, (char*)"wait-for-device", 0);
	}
...
top:
	...
	if (!strcmp(argv[0], "shell") || !strcmp(argv[0], "hell")) {
		...
		for (;;) {
			...
			if (persist) {
				fprintf(stderr, "\n- waiting for device -\n");
				adb_sleep_ms(1000);
				do_cmd(ttype, serial, (char*)"wait-for-device", 0);
			}
			else {
				if (h) {
					printf("\x1b[0m");
	...
	if (!strcmp(argv[0], "bugreport")) {
		...
		do_cmd(ttype, serial, (char*)"shell", "bugreport", 0);
		return 0;
	}
	...
	if (!strncmp(argv[0], "wait-for-", strlen("wait-for-"))) {
		char service[100]{};
		strcpy_s(service, 100, argv[0]);
		if (!strncmp(service, "wait-for-device", strlen("wait-for-device"))) {
			if (ttype == kTransportUsb) {
				strcpy_s(service, 100, "wait-for-usb");
			}
			else if (ttype == kTransportLocal) {
				strcpy_s(service, 100, "wait-for-local");
			}
			else {
				strcpy_s(service, 100, "wait-for-any");
			}
		}
...
static int do_cmd(transport_type ttype, char* serial, char *cmd, ...)
{
	...
	if (serial) {
		strcpy_s(argv[argc++], "-s");
		strcpy_s(argv[argc++], serial);
	}
	else if (ttype == kTransportUsb) {
		strcpy_s(argv[argc++], "-d");
	}
	else if (ttype == kTransportLocal) {
		strcpy_s(argv[argc++], "-e");
	}

	strcpy_s(argv[argc++], cmd);

	for (; argc < MAX_ARGV_LENGTH; ++argc)
	{
		strcpy_s(argv[argc], va_arg(ap, char*));
	}
	...
	return adb_commandline(argc, (char**)argv);
}
